...
public class AllInOneBase {
	...
	public AllInOneBase(){
		try{
			...
			String configPath = URLDecoder.decode(classLoader.getResource("/sampple/config/payment_conf.xml").getPath(), "UTF-8");
//			System.out.println(classLoader.getResource("/sampple").getPath());
//			String configPath = URLDecoder.decode(classLoader.getResource("/conf/payment_conf.xml").getPath(), "UTF-8");
			doc = EcpayFunction.xmlParser(configPath);
			/* when using testing code*/
//			String paymentConfPath = "./src/sampple/config/payment_conf.xml";
